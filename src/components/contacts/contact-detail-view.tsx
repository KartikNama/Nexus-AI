...
function canSendViaNexus(settings: WorkspaceEmailSettings | undefined) {
...
function ContactDetailContent({ contactId }: { contactId: string }) {
  ...
  const nexusSendReady = canSendViaNexus(emailSettings);
  ...
  const handleSendViaNexus = async () => {
    ...
    <Dialog open={introOpen} onOpenChange={setIntroOpen}>
      <DialogContent className={isMobileApp ? MOBILE_BOTTOM_SHEET : undefined}>
        <DialogHeader>
          ...
          <DialogDescription>
            Tell {contact.full_name} that you would like an introduction on Nexus AI. Choose your
            mail app or send from Nexus AI when email is configured.
          </DialogDescription>
        </DialogHeader>
        <div
      ...
      <DialogContent className={isMobileApp ? MOBILE_BOTTOM_SHEET : undefined}>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {apolloAccountLabel ? ` (${apolloAccountLabel})` : ""}, not Nexus AI.
          </DialogDescription>
        </DialogHeader>
        <div
          className={`flex flex-col gap-2 pb-[env(safe-area-inset-bottom)] sm:pb-0 ${isMobileApp ? "" : "sm:flex-row"}`}
      ...
      {outreach && (
        ...
        >
          ...
          <div className={`flex flex-wrap gap-2 ${isMobileApp ? "flex-col" : ""}`}>
            ...
            {contact.email && nexusSendReady ? (
              <Button
                size="sm"
                className={isMobileApp ? "w-full rounded-xl" : undefined}
                onClick={handleSendViaNexus}
