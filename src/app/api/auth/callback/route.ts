    ...
    if (connectorKey) {
      ...
      if (alreadyLinked && linkedToOtherUser) {
        return connectorsErrorRedirect(
          origin,
          `${providerLabel} account is already linked to a different Nexus AI user. Pick a different account in the chooser, or sign in with that ${providerLabel} account instead.`,
        );
      }
