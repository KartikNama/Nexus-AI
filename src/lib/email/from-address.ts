...
const SANDBOX_FROM = "Nexus AI <onboarding@resend.dev>";
...
export function getPlatformFromAddress() {
  ...
  if (!configured) {
    if (isProductionRuntime()) {
      throw new Error(
        "EMAIL_FROM is not set on the server. Add EMAIL_FROM=\"Nexus AI <you@yourdomain.com>\" to .env and restart.",
      );
    }
    return SANDBOX_FROM;
  }
  ...
  if (!email.includes("@")) {
    throw new Error(
      `EMAIL_FROM is invalid ("${configured}"). Quote the value in .env, e.g. EMAIL_FROM="Nexus AI <contact@yourdomain.com>".`,
    );
  }
